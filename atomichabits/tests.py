...
class HabitTestCase(APITestCase):
    ...
    def test_related_habit_validation(self):
        ...
        # Проверяем, что список ошибок содержит одну ошибку

        self.assertEqual(len(errors), 1)

        # Сравниваем первую ошибку в списке

        self.assertEqual(
            errors[0],
            "Связанная привычка должна быть полезной, если привычка неприятная.",
        )
    ...
    def test_rel_habit_and_award_validation(self):
        ...
        self.assertEqual(
            errors[0],
            "Недопустимый первичный ключ "
            f"'{data['related_habit']}' - объект не существует.",
        )
